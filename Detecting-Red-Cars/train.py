...
'''def train as a function'''
def train(train_image_cut,train_cut):
    best_knn = 2
    '''Import DataSet'''
    ...
    column_cut_len = 10
    row_cut_len = 10
    (sample_array,sample_position_array) = array_spilt(train_image_cut,column_cut_len,row_cut_len)
    ...
    for i in range(len(k)):
        ...
        #test accuracy 
        accuracy_KNN_2D = accuracy_score(label_valid_2D, predictions_KNN_2D)
        accuracy_KNN_2D_array.append(accuracy_KNN_2D)
    #    print('\nThe accuracy of KNN classifier is: ', accuracy_KNN_2D*100, '%')
    
    plt.figure()
    plt.plot(k,accuracy_KNN_2D_array,'bo-')
    plt.ylabel('accuracy') #label x and y axes
    plt.xlabel('k')
    plt.legend(['red_car'])
    plt.title('accuracy KNN for red car' ) 
...
#    show_red_car(train_cut,position_array)
    
    
    '''color pass filter'''
    red_car_filter = cut_figure(train_image_cut,position_array[:,1],position_array[:,0])
    #for i in range(len(red_car_filter)):
    #    plt.figure()
    #    plt.imshow(red_car_filter[i])
    
    position_filter = color_pass_filter(red_car_filter,position_array)
    img = show_red_car(train_cut,position_filter)
    img.save('train_circle.jpg')    
        
    return best_knn,position_filter
